# 빙하가 깨지면서 스노우타운에 떠내려 온 죠르디는 인생 2막을 위해 주택 건축사업에 뛰어들기로 결심하였습니다. 죠르디는 기둥과 보를 이용하여 벽면 구조물을 자동으로 세우는 로봇을 개발할 계획인데,
...
def testB(pillar, beam, x, y, n):
    if y-1 >= 0 and pillar[x][y-1] == 1:
        return True
    if y-1 >= 0 and x+1 <= n and pillar[x+1][y-1] == 1:
        return True
    if x-1 >= 0 and x+1 <= n and beam[x-1][y] == 1 and beam[x+1][y] == 1:
        return True
    return False


def testP(pillar, beam, x, y):
    if y == 0:
        return True
    if x-1 >= 0 and beam[x-1][y] == 1:
        return True
    if beam[x][y] == 1:
        return True
    if y-1 >= 0 and pillar[x][y-1] == 1:
...
def solution(n, build_frame):
    ...
    # beam and pillar check build_frame
    for x, y, a, b, in build_frame:
        if b == 1:  # install
            if a == 0:  # pillar
                if testP(pillar, beam, x, y):
                    pillar[x][y] = 1
            else:  # beam
                if testB(pillar, beam, x, y, n):
                    beam[x][y] = 1
        else:  # delete
            if a == 0:  # pillar
                pillar[x][y] = 0
                ...
                for i in range(N):
                    for j in range(N):
                        test &= beam[i][j] == 0 or testB(pillar, beam, i, j, n)
                        test &= pillar[i][j] == 0 or testP(pillar, beam, i, j)
                if test is False:
                    pillar[x][y] = 1
            else:  # beam
                beam[x][y] = 0
